...
function GMM(n::Int, x::DataOrMatrix{T}; method::Symbol=:kmeans, kind=:diag,
             nInit::Int=50, nIter::Int=10, nFinal::Int=nIter, sparse=0, parallel=true) where T <: AbstractFloat
    if n < 2
        return GMM(x, kind=kind, parallel=parallel)
    elseif method == :split
...
GMM(n::Int, x::Vector{T}; method::Symbol=:kmeans, nInit::Int=50, nIter::Int=10, nFinal::Int=nIter, sparse=0, parallel=true) where T <: AbstractFloat = GMM(n, reshape(x, length(x), 1); method=method, kind=:diag, nInit=nInit, nIter=nIter, nFinal=nFinal, sparse=sparse, parallel=parallel)
...
function GMMk(n::Int, x::DataOrMatrix{T}; kind=:diag, nInit::Int=50, nIter::Int=10, sparse=0, parallel=true) where T <: AbstractFloat
    nₓ, d = size(x)
    ...
    sanitycheck!(gmm)
    em!(gmm, x; nIter=nIter, sparse=sparse, parallel=parallel) # HACK
    return gmm
end
...
function GMM2(n::Int, x::DataOrMatrix; kind=:diag, nIter::Int=10, nFinal::Int=nIter, sparse=0, parallel=true)
...
function em!(gmm::GMM, x::DataOrMatrix; nIter::Int = 10, varfloor::Float64=1e-3, sparse=0, debug=1, parallel=true)
    ...
    for i in 1:nIter
        ## E-step
        nₓ, ll[i], N, F, S = stats(gmm, x; parallel)
        ## M-step
        gmm.w = N / nₓ
        gmm.μ = F ./ N
        if gmmkind == :diag
